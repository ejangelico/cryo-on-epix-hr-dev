...
class Board(pyrogue.Root):
    ...
    def setPulserManual(self, pulser_value):
        print("Setting pulser manually to " + str(pulser_value))
        self.EpixHRGen1Cryo.CryoAsic0.Pulser.set(pulser_value)
...
with Board(cmd, dataWriter, srp, timeout=timeout_time) as cryoAsicBoard:
    try:
        if ( args.type == 'dataFile' or args.type == 'SIM'):
            ...
            print("Finished doing init sequence")

            #do a loop setting the pulser value (which merits a response from the ASIC)
            #every delay cycle. Cmd+c will take us out of this try statement
            cmd_delay = 1
            pulser_value = 880 #decimal of 0x370 
            while True:
                cryoAsicBoard.setPulserManual(pulser_value)
